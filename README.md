...
- **智能缓存**：优先使用数据库缓存，减少API调用，数据超过30分钟自动刷新
...
2. **过期检查**：如果缓存数据超过30分钟，自动从API获取新数据
...
- 缓存数据在30分钟内有效，超过30分钟会自动从API获取新数据
...
1. **智能缓存**：
   - 优先从数据库获取今日及之后的数据
   - 如果缓存数据在30分钟内，直接使用缓存，不调用API
   - 如果缓存数据超过30分钟，自动从API获取新数据并更新缓存
   - 数据库查询失败时，自动回退到使用API获取数据
