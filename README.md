...
- **智能缓存**：优先使用数据库缓存，减少API调用，数据超过指定时间（默认30分钟，可配置）自动刷新
...
2. **过期检查**：如果缓存数据超过指定时间（默认30分钟，可通过 `CACHE_EXPIRE_MINUTES` 环境变量配置），自动从API获取新数据
...
| `CACHE_EXPIRE_MINUTES` | ⭕ 可选 | `30` | `30` / `60` / `120` 等 | 缓存过期时间（分钟），默认30分钟。超过此时间后，系统会自动从API获取新数据 |
...
#   - 默认值: 30分钟
#   - 说明: 控制缓存数据的过期时间，超过此时间后会自动从API获取新数据
#   - 示例: 设置为60表示缓存60分钟后过期
```
...
- 缓存数据在指定时间内有效（默认30分钟，可通过 `CACHE_EXPIRE_MINUTES` 环境变量配置），超过此时间会自动从API获取新数据
...
3. **配置环境变量**：
   ...
   - `CACHE_EXPIRE_MINUTES`（可选，默认：`30`，缓存过期时间（分钟））
...
1. **智能缓存**：
   - 优先从数据库获取今日及之后的数据
   - 如果缓存数据在指定时间内（默认30分钟，可通过 `CACHE_EXPIRE_MINUTES` 环境变量配置），直接使用缓存，不调用API
